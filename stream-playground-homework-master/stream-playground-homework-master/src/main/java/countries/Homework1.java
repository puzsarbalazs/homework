...
public class Homework1 {
    ...
    public Optional<String> streamPipeline2() {
        return countries.stream()
                .map(country -> country.getName().toLowerCase())
                .filter(country -> country.contains("island"))
                .findFirst();
    }
    ...
    public void streamPipeline3() {
        countries.stream()
                .filter(country -> country.getName().toLowerCase().charAt(0) == country.getName().toLowerCase().charAt(country.getName().toLowerCase().length() - 1))
...
ed by the language code "es").
    ...
    public long streamPipeline9() {
        return countries.stream()
                .map(country -> country.getTranslations().keySet())
                .filter(country -> !(country.contains("es")))
                .count();
    }
